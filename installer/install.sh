...
URL="https://github.com/Benefranko/Barcode-Scanner-Feinkost-.git"
...
if sudo apt install python3-PySide6.*; then
  echo -e "    -> OK\n"
else
  echo "    -> FAILED --> EXIT()"
  exit
...
# Ändere auch bei erstellten Rechte
sudo chown -R "${SUDO_USER}:${SUDO_USER}" "/home/${SUDO_USER}/Barcode-Scanner-Feinkost-/"

read -r -p "Reboot ? [J/n]: " p
